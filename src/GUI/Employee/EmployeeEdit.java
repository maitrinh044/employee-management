...
public class EmployeeEdit extends javax.swing.JFrame {
    ...
    private EmployeeDTO selectedEm;
     ...
     * @param em
     */
    public EmployeeEdit(EmployeeDTO em) {
        initComponents();

        selectedEm = new EmployeeDTO(em);

        txtName.setText(em.getFullName());
        birthday.setDate(em.getBirthday());
        txtPhone.setText(em.getPhoneNumber());
        txtAdr.setText(em.getAddress());

        cbbGender.addItem("Nữ");
        cbbGender.addItem("Nam");
        cbbGender.setSelectedItem(em.getGender());

        List<PositionDTO> psList = psBUS.getAllPositions();
        String selectedPositionName = null;
        for (PositionDTO ps : psList) {
            cbbPosition.addItem(ps.getPositionName());
            if (ps.getPositionId() == em.getPositionId()) {
                selectedPositionName = ps.getPositionName();
            }
        }
        if (selectedPositionName != null) {
            cbbPosition.setSelectedItem(selectedPositionName);
        }

        List<DepartmentDTO> dpList = dpBUS.getAllDepartments();
        String slDpName = null;
        for (DepartmentDTO dp : dpList) {
            cbbDepart.addItem(dp.getDepartmentName());
            if (dp.getDepartmentId() == em.getDepartmentId()) {
                slDpName = dp.getDepartmentName();
            }
        }
        if (slDpName != null) {
            cbbDepart.setSelectedItem(slDpName);
        }

        cbbStatus.addItem("Hoạt động");
        cbbStatus.addItem("Không hoạt động");
        cbbStatus.setSelectedItem(em.isStatus() ? "Hoạt động" : "Không hoạt động");
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        boolean status = cbbStatus.getSelectedItem() == "Hoạt động" ? true : false;
        ...
        if (!phone.equals(selectedEm.getPhoneNumber()) && emBUS.isExistPhoneNumber(phone)) {
        ...
        EmployeeDTO em = new EmployeeDTO(selectedEm.getEmployeeId(), name, birth, gender, phone, adr, psDTO.getPositionId(), dpDTO.getDepartmentId(), status);
        if (emBUS.updateEmployee(em)) {
